package Inflectra::SpiraTest::Addons::Aggregator::SpiraAggregator;
...
use constant EXECUTION_STATUS_ID_PASSED => 2;
use constant EXECUTION_STATUS_ID_FAILED => 1;
use constant EXECUTION_STATUS_ID_NOT_RUN => 3;
use constant EXECUTION_STATUS_ID_CAUTION => 6;
use constant EXECUTION_STATUS_ID_BLOCKED => 5;

@ISA = qw(TAP::Object);

=head1 NAME

Inflectra::SpiraTest::Addons::Aggregator::SpiraAggregator - Replaces the built-in aggregator to also report the results back to SpiraTest via. web service
...
Replaces the built-in aggregator to also report the results back to SpiraTest via. web service

=head1 SYNOPSIS

 use Inflectra::SpiraTest::Addons::Aggregator::SpiraAggregator;
 my $harness = Inflectra::SpiraTest::Addons::Aggregator::SpiraAggregator->new( \%args );
...
sub add
{
    ...
    $self->{"testName"} = $description;
    my $executionStatusId = EXECUTION_STATUS_ID_NOT_RUN;
    if ($parser->actual_failed > 0)
    {
      $executionStatusId = EXECUTION_STATUS_ID_FAILED;
    }
    if ($parser->actual_passed > 0 && $executionStatusId == EXECUTION_STATUS_ID_NOT_RUN)
    {
      $executionStatusId = EXECUTION_STATUS_ID_PASSED;
    }
    if ($parser->todo > 0 && $executionStatusId != EXECUTION_STATUS_ID_NOT_RUN)
    {
      #since we have todo-s and no other failures, log as blocked
      $executionStatusId = EXECUTION_STATUS_ID_BLOCKED;
    }
    $self->{"testStatus"} = $executionStatusId;
